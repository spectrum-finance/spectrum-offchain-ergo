...
fn deploy_pool_chain_transaction(
    ...
    // TX 1: Mint pool NFT -------------------------------------------------------------------------
    let inputs = filter_tx_outputs(tx_0.outputs.clone().to_vec());
...
fn test_deploy_pool_chain_tx() {
    ...
    let pool = Pool::try_from_box(pool_init_tx.outputs.get(0).unwrap().clone()).unwrap();
    let staking_bundle = StakingBundle::try_from_box(pool_init_tx.outputs.get(2).unwrap().clone()).unwrap();
}
